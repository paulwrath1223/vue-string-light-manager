...
void passCredentials(String uid)
{
  ...
  navigateTo(URL); // replace with real function and check formatting on URL
...
void checkDistress()
{
  WiFi.disconnect();   // dont know if this is neccesary. remove if possible
  delay(100);
  ...
  else
  {
    ...
    for (int i = 0; i < n; ++i)
    {
      ...
      if(currentSSID.split("/")[0] == "WifiLightController")    //not too sure about ".split" in c++
      {
        const String currentUID = currentSSID.split("/")[1];
